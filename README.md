...
<!-- BADGES -->
<a href="#"><img src="https://img.shields.io/badge/Phiên_bản-3.2.0-EE0033?style=for-the-badge&logo=semanticweb&logoColor=white" alt="Version"/></a>
&nbsp;
...
&nbsp;
<img src="https://img.shields.io/badge/Smart_Skip-v3.2-6366f1?style=flat-square"/>
...
**📂 2 nguồn mã đơn linh hoạt** *(Mới v3.2)*
- **File Excel (.xlsx)** — load file tồn → group theo `TEN_KHGUI` → chọn 1 khách → tool tự lấy `MA_PHIEUGUI` của khách đó.
- **Dán mã trực tiếp** — paste danh sách mã (mỗi dòng / phẩy / tab) khi cần chạy nhanh. Tự khử trùng lặp.

**🛡️ Smart Skip + an toàn DOM** *(v3.2)*
Tự phát hiện đơn không hỗ trợ sửa giờ → bỏ qua sạch. Đã vá lỗi đặt **nhầm ngày hàng loạt** (chọn theo TEXT thay vì id động) và lỗi recovery có thể **hủy đơn hàng thật**.

**📊 Báo cáo kết quả** *(Mới v3.2)*
Cuối phiên hiện card 2 tab **"Thất bại N / Thành công M"** — list từng mã + lý do skip, nút **Copy** ra clipboard để theo dõi/xử lý thủ công.
...
- Mạng chậm khiến khung giờ "Cả ngày" không tải kịp *(v3.2)*
...
Bạn có thể copy danh sách Thất bại để xử lý thủ công sau.
